...
MAX_THRESHOLD = 3000
MAX_GAIN = 63
MAX_MIN_LAP = 0x7FFFFFFFFFFFFFFF

FREQUENCY_BANDS = ['F','R', 'E', 'B', 'A', 'L']
...
class PingEvent(LapRFEvent):
    "A LapRF Ping event"

    def __init__(self: "PingEvent"):
        super().__init__("ping")
        self.ping: Optional[float] = None

    def is_valid(self: "PingEvent") -> bool:
        return (isinstance(self.ping, float))
...
Event = Union[RFSetupEvent, PingEvent, PassingEvent, SettingsEvent, StatusEvent, TimeEvent]
...
def _decode_ping_record(record: Decoder) -> PingEvent:
    event = PingEvent()
    while record.pos < record.length:
        signature = record.decode_field_signature()
        if signature == EOR:
            break
        elif signature == PingField.PING:
            event.ping = record.decode_f32_field()
        else:
            record.skip_unknown_field(signature)
    return event
...
def _decode_record(buffer: bytes):
    ...
    if record.rec_type == RecordType.RF_SETUP:
        ...
        pass
    elif record.rec_type == RecordType.PING:
        return _decode_ping_record(record)
...
def encode_ping_record(ping: float) -> bytes:
    """Encode a LapRF RF record to send a ping

    Attributes:
      ping: float - Data to send and receive.
    """
    return (Encoder(RecordType.PING)
            .encode_f32_field(PingField.PING, ping)
            .finish())
